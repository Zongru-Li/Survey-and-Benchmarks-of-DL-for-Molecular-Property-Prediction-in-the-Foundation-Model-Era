...
# adme_rppb dataset experiments with dmpnn
# Dataset: adme_rppb (ADME, regression)
# Splits: scaffold, butina, umap, time
# Epochs: 501

python src/run.py --config configs/dmpnn.yaml --dataset adme_rppb --split scaffold --epochs 501 &
python src/run.py --config configs/dmpnn.yaml --dataset adme_rppb --split butina --epochs 501 &
