...
# adme_rlm dataset experiments with dmpnn
# Dataset: adme_rlm (ADME, regression)
# Splits: scaffold, butina, umap, time
# Epochs: 501

python src/run.py --config configs/dmpnn.yaml --dataset adme_rlm --split scaffold --epochs 501 &
python src/run.py --config configs/dmpnn.yaml --dataset adme_rlm --split butina --epochs 501 &
