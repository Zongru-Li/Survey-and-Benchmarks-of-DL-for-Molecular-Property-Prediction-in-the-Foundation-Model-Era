...
# adme_hppb dataset experiments with ngram_xgb
# Dataset: adme_hppb (ADME, regression)
# Splits: scaffold, butina, umap, time
# Epochs: 501

python src/run.py --config configs/ngram_xgb.yaml --dataset adme_hppb --split scaffold --epochs 501 &
python src/run.py --config configs/ngram_xgb.yaml --dataset adme_hppb --split butina --epochs 501 &
