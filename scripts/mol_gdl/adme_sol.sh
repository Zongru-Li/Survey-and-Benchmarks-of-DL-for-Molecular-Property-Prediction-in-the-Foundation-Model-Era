...
# adme_sol dataset experiments with mol_gdl
# Dataset: adme_sol (ADME, regression)
# Splits: scaffold, butina, umap, time
# Epochs: 501

python src/run.py --config configs/mol_gdl.yaml --dataset adme_sol --split scaffold --epochs 501 &
python src/run.py --config configs/mol_gdl.yaml --dataset adme_sol --split butina --epochs 501 &
