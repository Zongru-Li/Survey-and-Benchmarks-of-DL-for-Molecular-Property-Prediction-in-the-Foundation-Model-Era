...
# adme_sol dataset experiments with pretrain_gnn
# Dataset: adme_sol (ADME, regression)
# Splits: scaffold, butina, umap, time
# Epochs: 501

python src/run.py --config configs/pretrain_gnn.yaml --dataset adme_sol --split scaffold --epochs 501 &
python src/run.py --config configs/pretrain_gnn.yaml --dataset adme_sol --split butina --epochs 501 &
