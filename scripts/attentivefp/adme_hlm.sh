...
# adme_hlm dataset experiments with attentivefp
# Dataset: adme_hlm (ADME, regression)
# Splits: scaffold, butina, umap, time
# Epochs: 501

python src/run.py --config configs/attentivefp.yaml --dataset adme_hlm --split scaffold --epochs 501 &
python src/run.py --config configs/attentivefp.yaml --dataset adme_hlm --split butina --epochs 501 &
