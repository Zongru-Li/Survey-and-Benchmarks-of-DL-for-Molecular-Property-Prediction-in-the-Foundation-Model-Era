...
from src.utils.pyg_data import create_pyg_dataloader
...
def main():
    ...
    if is_pyg:
        train_loader, val_loader, test_loader = create_pyg_dataloader(config)
    else:
    ...
    write_results(
        ...
        include_stddev=args.stddev,
    )

    if args.stddev:
        print(
            f"[INFO] {metric_name}: {metric_value:.4f}, STD_DEV: {std_dev:.4f}",
            flush=True,
        )
        print(f"mean: {metric_value:.4f}", flush=True)
        print(f"std: {std_dev:.4f}", flush=True)
    else:
        print(f"[INFO] {metric_name}: {metric_value:.4f}", flush=True)
        print(f"mean: {metric_value:.4f}", flush=True)
    return 0
