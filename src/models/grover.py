...
class GroverTransEncoder(nn.Module):
    def __init__(
        self, hidden_size, num_layers, num_heads, dropout=0.1, activation="ReLU"
    ):
        super(GroverTransEncoder, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers

        self.atom_encoder = GroverMPNEncoder(
            hidden_size, num_layers, dropout, activation
        )
        self.bond_encoder = GroverMPNEncoder(
            hidden_size, num_layers, dropout, activation
        )
...
class GROVER(BaseModel):
    def __init__(
        ...
        self.atom_ffn = self._create_ffn(
            self.emb_dim, feat_dim, out, ffn_num_layers, drop_ratio
        )
        self.bond_ffn = self._create_ffn(
            self.emb_dim, feat_dim, out, ffn_num_layers, drop_ratio
        )
    ...
    def forward(self, data):
        ...
        if edge_attr is not None and edge_attr.numel() > 0:
            edge_embed = self.edge_embedding1(edge_attr[:, 0]) + self.edge_embedding2(
                edge_attr[:, 1]
            )
        else:
            edge_embed = None
