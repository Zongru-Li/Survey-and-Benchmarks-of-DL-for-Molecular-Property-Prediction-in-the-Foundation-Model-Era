...
class MPNEncoder(nn.Module):
    def __init__(
        self, hidden_size, depth, dropout=0.1, activation="ReLU", atom_messages=True
    ):
...
class DualMPNEncoder(nn.Module):
    def __init__(self, hidden_size, depth, dropout=0.1, activation="ReLU"):
        ...
        self.atom_encoder = MPNEncoder(
            hidden_size, depth, dropout, activation, atom_messages=True
        )
        self.bond_encoder = MPNEncoder(
            hidden_size, depth, dropout, activation, atom_messages=False
        )
...
class CDMVGNN(BaseModel):
    def __init__(
        ...
        self.encoder = DualMPNEncoder(
            hidden_size=self.emb_dim, depth=num_layers, dropout=drop_ratio
        )
        ...
        self.mol_atom_ffn = self._create_ffn(
            self.emb_dim, feat_dim, out, ffn_num_layers, drop_ratio
        )
        self.mol_bond_ffn = self._create_ffn(
            self.emb_dim, feat_dim, out, ffn_num_layers, drop_ratio
        )
