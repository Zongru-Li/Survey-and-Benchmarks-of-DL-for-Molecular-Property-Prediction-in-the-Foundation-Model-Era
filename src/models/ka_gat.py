...
class Gat_Kan_layer(nn.Module):
    def __init__(
        ...
        self.fc_node = nn.Linear(
            in_node_feats + in_edge_feats, out_node_feats * num_heads, bias=True
        )
        self.fc_ni = nn.Linear(in_node_feats, out_edge_feats * num_heads, bias=False)
        self.fc_fij = nn.Linear(in_edge_feats, out_edge_feats * num_heads, bias=False)
        self.fc_nj = nn.Linear(in_node_feats, out_edge_feats * num_heads, bias=False)
        self.attn = nn.Parameter(torch.FloatTensor(size=(1, num_heads, out_edge_feats)))
        self.output_node = KAN_linear(
            out_node_feats, out_node_feats, grid_size, addbias=True
        )
        self.output_edge = KAN_linear(
            out_edge_feats, out_edge_feats, grid_size, addbias=True
        )
        self.edge_kan = KAN_linear(
            out_edge_feats * num_heads,
            out_edge_feats * num_heads,
            gridsize=1,
            addbias=True,
        )
        self.node_kan = KAN_linear(
    ...
    def forward(self, graph, nfeats, efeats, get_attention=False):
        with graph.local_scope():
            ...
            graph.send_and_recv(
                graph.edges(), self.message_func, reduce_func=self.reduce_func
            )
...
class KA_GAT(nn.Module):
    def __init__(
        ...
        self.node_kan_line = KAN_linear(
            in_node_dim, hidden_dim, gride_size, addbias=False
        )
        self.edge_kan_line = KAN_linear(
            in_edge_dim, hidden_dim, gride_size, addbias=False
        )
