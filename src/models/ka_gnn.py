...
class NaiveFourierKANLayer(nn.Module):
    ...
    def forward(self, g, x):
        with g.local_scope():
            ...
            g.update_all(
                message_func=self.fourier_transform,
    ...
    def fourier_transform(self, edges):
        ...
        k = torch.reshape(
            torch.arange(1, self.gridsize + 1, device=src_feat.device),
            (1, 1, 1, self.gridsize),
        )
        ...
        cos_kx = torch.reshape(
            cos_kx, (1, src_feat.shape[0], src_feat.shape[1], self.gridsize)
        )
        sin_kx = torch.reshape(
            sin_kx, (1, src_feat.shape[0], src_feat.shape[1], self.gridsize)
        )

        # Perform Fourier transform using einsum
        m = torch.einsum(
            "dbik,djik->bj", torch.concat([cos_kx, sin_kx], axis=0), self.fouriercoeffs
        )
