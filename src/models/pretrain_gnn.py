...
class PretrainGNN(BaseModel):
    def __init__(
        ...
        self.gnn = GNN(num_layers, self.emb_dim, JK=JK, drop_ratio=drop_ratio, gnn_type=gnn_type)
        ...
        if pooling == "sum":
            ...
            self.pool = GlobalAttention(gate_nn=nn.Linear(self.emb_dim, 1))
        elif pooling.startswith("set2set"):
            set2set_iter = int(pooling[-1])
