    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lins = nn.ModuleList(
            [
                KANLinear(
                    in_channels, out_channels, grid_size, spline_order, 0.1, 1.0, 1.0
                )
                for _ in range(K)
            ]
        )
...
class GraphKAN(BaseModel):
    def __init__(
        ...
        pool_dim = (
            hidden_dims[-1] * 2 if pooling.startswith("set2set") else hidden_dims[-1]
        )
        self.graph_pred_linear = KANLinear(
            pool_dim, out, grid_size=grid_size, spline_order=spline_order
        )
    ...
    def forward(self, data):
        ...
        if x.dtype == torch.long:
            x = self.x_embedding1(x[:, 0]) + self.x_embedding2(
                x[:, 1] if x.size(1) > 1 else torch.zeros_like(x[:, 0])
            )
