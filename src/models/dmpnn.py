    ...
    def forward(self, g, atom_feats, bond_feats):
        with g.local_scope():
            ...
            for _ in range(self.depth - 1):
                ...
                g.send_and_recv(g.edges(), fn.copy_e("h_edge", "m"), fn.sum("m", "tmp"))
            ...
            g.edata["h_final"] = h
            g.send_and_recv(
                g.edges(), fn.copy_e("h_final", "m"), fn.sum("m", "atom_msg")
            )
...
class DMPNN(BaseModel):
    ...
    def forward(self, g, h):
        atom_feats = h
        bond_feats = g.edata.get(
            "feat", torch.zeros(g.num_edges(), self.bond_fdim, device=h.device)
        )
