...
def train_epoch_pyg(
    model, device, train_loader, optimizer, loss_fn, is_dual_output=False
):
    model.train()
    total_train_loss = 0.0
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        mask = (y != -1).to(
            dtype=out.dtype if not isinstance(out, tuple) else out[0].dtype
        )
        y_clean = torch.where(y == -1, torch.zeros_like(y), y)
...
def train_model(
    ...
    for i in range(iterations):
        ...
        for epoch in range(epochs):
            if is_pyg:
                train_loss = train_epoch_pyg(
                    model, device, train_loader, optimizer, loss_fn, is_dual_output
                )
                if val_loader:
                    val_loss = validate_epoch_pyg(
                        model, device, val_loader, loss_fn, is_dual_output
                    )
                    print(
                ...
                if is_regression:
                    score = predict_pyg_regression(
                        model, device, test_loader, is_dual_output
                    )
                else:
                    score = predict_pyg(model, device, test_loader, is_dual_output)
            elif is_gnn_model:
                train_loss = train_epoch_gnn(
            ...
            if epoch == epochs - 1:
                print(
                    f"the best result up to {i+1}-loop is {best_score:.4f}.", flush=True
                )
