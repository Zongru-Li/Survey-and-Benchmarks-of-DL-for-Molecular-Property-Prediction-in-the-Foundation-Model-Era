...
class UMAPSplitter(Splitter):
    ...
    def split(self, dataset, frac_train=0.8, frac_valid=0.1, frac_test=0.1, seed=None):
        ...
        try:
            ...
            raise ImportError(
                "umap-learn is required for UMAPSplitter. Install with: pip install umap-learn"
            )
        ...
        for cluster_id in range(n_clusters):
            cluster_to_indices[cluster_id] = np.where(cluster_labels == cluster_id)[
                0
            ].tolist()

        clusters_with_size = [
            (cid, len(indices)) for cid, indices in cluster_to_indices.items()
        ]
